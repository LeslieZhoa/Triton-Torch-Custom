...
class EqualConvTranspose2d(nn.Module):
    def __init__(
        ...
        self.weight = nn.Parameter(
            torch.randn(in_channel, out_channel, kernel_size, kernel_size) * 1e-2
        )
...
class Generator(nn.Module):
    def __init__(
        ...
        if not big_model:
            ...
            ch_multiplier = (8, 16, 16, 12, 10, 8, 8, 4)
            upsample = []
            for i in range(4):
        ...
        self.jiajie_reso = jiajie_reso

    def forward(self, structure, texture, noises=None, trun_idx=[], trun_texture=None, high=False, jiajie_flag=False):
        if noises is None:
        ...
        for layer, noise in zip(self.layers, noises):
            inp_texture = texture
            # if idx in trun_idx:
            #     inp_texture = trun_texture
            out = layer(out, inp_texture, noise)
            # if out.shape[2] == self.jiajie_reso:
            #     fmap = out
            # if high and out.shape[2] == 128:
            #     high_fmap = out
            # idx += 1
...
class Discriminator(nn.Module):
    def __init__(self, size, channel_multiplier=1, blur_kernel=(1, 3, 3, 1)):
        ...
        similar_size = size
        if size > 256 and size < 512:
            similar_size = 256

        convs = [ConvLayer(3, channels[similar_size], 1)]

        log_size = int(math.log(size, 2))


        in_channel = channels[similar_size]
        ...
        feat_size = int(size/128)

        self.final_linear = nn.Sequential(
            EqualLinear(channels[4] * feat_size * feat_size, channels[4], activation="fused_lrelu"),
